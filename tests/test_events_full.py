...
def test_event_process_insert(here, connection, metadata, gebieden_schema):
    """Prove that event gets inserted."""
    events_path = here / "files" / "data" / "bouwblokken.gobevents"
    importer = EventsProcessor(
        [gebieden_schema], 28992, connection, local_metadata=metadata, truncate=True
    )
    importer.load_events_from_file(events_path)
    records = [dict(r) for r in connection.execute("SELECT * from gebieden_bouwblokken")]
...
def test_event_process_update(here, connection, metadata, gebieden_schema):
...
def test_event_process_delete(here, connection, metadata, gebieden_schema):
...
def test_event_process_1n_relation_insert(here, connection, metadata, gebieden_schema):
    ...
    # about the relation
    through_records = [
        dict(r) for r in connection.execute("SELECT * from gebieden_bouwblokken_ligt_in_buurt")
    ]
...
def test_event_process_1n_relation_delete(here, connection, metadata, gebieden_schema):
...
def test_event_process_nm_relation_insert(
    ...
    records = [
        dict(r)
        for r in connection.execute("SELECT * from gebieden_ggwgebieden_bestaat_uit_buurten")
    ]
    assert len(records) == 1
...
def test_event_process_nm_relation_update(here, connection, metadata, gebieden_schema):
...
def test_event_process_nm_relation_delete(here, connection, metadata, gebieden_schema, salogger):

...
def test_import_schema(db_url, schemas_mock, session):
    ...
    dataset: Dataset = session.query(Dataset).get("afvalwegingen")
    assert dataset.date_created == datetime(2020, 1, 13)

    # Check that all tables are created.
    table_count: int = session.query(Table).filter(Table.dataset_id == dataset.id).count()
